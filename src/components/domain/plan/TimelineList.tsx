...
export default function TimelineList({ places, onReorder, onDelete, onOpenSearch }: TimelineListProps) {
  ...
  if (!mounted) return <div className="p-8 text-center text-gray-500">타임라인 로딩중...</div>;
    ...
    <div className="flex flex-col w-full h-full bg-transparent overflow-y-auto">
      <div className="flex items-center justify-between mb-4 mt-1 px-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] text-gray-500 font-medium">{places.length}개 장소</span>
        </div>
      ...
      <div className="flex flex-col relative w-full pt-1 pb-16">
                          ...
                          <div className="flex-1 ml-4 bg-white rounded-xl border border-gray-200 p-4 shadow-[0_2px_8px_rgba(0,0,0,0.02)] group hover:border-gray-300 transition-colors">
                            <div className="flex justify-between items-start">
                              ...
                              <div className="flex items-center text-gray-400 gap-0.5 mt-[-4px] mr-[-4px]">
                                ...
                                {/* X 버튼 누르면 삭제 함수 실행 */}
                                <button onClick={() => onDelete(place.id)} className="p-1.5 hover:bg-gray-50 rounded-md text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors">
                                  <X className="w-4 h-4" />
                                </button>
                              </div>
                            </div>
        ...
        <div className="mt-2 ml-11 mb-8">
          <CommonButton onClick={onOpenSearch} variant="outline" className="w-full !rounded-xl !py-3 !text-[13px] font-semibold flex justify-center !border-purple-200 !text-purple-600 bg-purple-50 hover:bg-purple-100 shadow-sm transition-colors cursor-pointer">
            <Plus className="w-4 h-4 mr-1.5" /> + 장소 추가하기
          </CommonButton>
        </div>
